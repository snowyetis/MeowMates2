module UserSurveys
  module SurveysHelper
    ...
    def new_attempt_path
      new_user_surveys_attempt_path
    end

    def new_survey_path
      new_user_surveys_survey_path
    end

    def edit_survey_path(resource)
      edit_user_surveys_survey_path(resource)
    end

    def attempt_scope(resource)
      if action_name =~ /new|create/
        user_surveys_attempts_path(resource)
      elsif action_name =~ /edit|update/
        user_surveys_attempt_path(resource)
      end
    end

    def survey_scope(resource)
      if action_name =~ /new|create/
        user_surveys_surveys_path(resource)
      elsif action_name =~ /edit|update/
        user_surveys_survey_path(resource)
      end
    end
